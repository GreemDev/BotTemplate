...
namespace Voltemplate.Interactions
{
    ...
    public abstract class ApplicationCommand
    {
        ...
        public SlashCommandSignature SlashSignature { get; set; } = SlashCommandSignature.Command();

        public void Signature(Action<Options> optionsProducer)
        {
            SlashSignature = SlashSignature.Options(optionsProducer);
        }

        public virtual Task HandleSlashCommandAsync(SlashCommandContext ctx) => Task.CompletedTask;

        public virtual Task HandleUserCommandAsync(UserCommandContext ctx) => Task.CompletedTask;

        public virtual Task HandleMessageCommandAsync(MessageCommandContext ctx) => Task.CompletedTask;
